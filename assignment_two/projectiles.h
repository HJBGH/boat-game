...
better with regards to balance and playability*/
...
typedef struct
{
     ...
     * d tracks x, y and z components of current velocity,
     * acceleration is handled in update functions */
} Proj2Vec3f; /*a projectile made of 2x vec2f structs*/

void drawTraj(const Proj2Vec3f * shell);/*draw trajectory of projectile*/
void drawProj(const Proj2Vec3f * shell);/*draw a projectile*/
void updateProj(Proj2Vec3f * shell);/*update a projectile*/
void updateDefProj(Proj2Vec3f * dshell);/*wrapper for updateProj, also updates the
...
extern Proj2Vec3f *mag[MAG_DEPTH];/*MAG_DEPTH is defined in includes.h*/
